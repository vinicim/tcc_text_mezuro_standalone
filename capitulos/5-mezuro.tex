...
O Mezuro, desde sua criação, utiliza um web service chamado Kalibro Metrics. Isso permite ao Mezuro: 
...
Acompanhando a evolução do framework Rails, visando os novos recursos, além do suporte da comunidade\footnote{\url{http://rubyonrails.org/}} desse framework, que favorece a utilização das últimas versões, a equipe de desenvolvimento da plataforma Mezuro decidiu atualiza-la para as novas versões 2 e 4 do Ruby e do Rails respectivamente. 

O Noosfero desfavorece a evolução não só por seus recursos obsoletos, mas também por possuir muitos recursos desnecessários para uma ferramenta de monitoramento de código-fonte. Entre esses recursos estão: blog, fórum, upload de arquivos, CMS, chat entre outros. Evolução de software também significa retirar funcionalidades que não se encaixam mais ao ambiente que o software está inserido. Foi exatamente nisso que a equipe de desenvolvimento também levou em consideração para evoluir o Mezuro.
...
%Nova arquitetura
%Inserir anexo do formulário de pesquisa realizado com a equipe do Mezuro
%Inserir resultados da pesquisa realizada
